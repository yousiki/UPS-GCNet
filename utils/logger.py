...
class Logger(object):
    ...
    def _setup_dirs(self, args):
        ...
        file_dir = os.path.join(
            args.log_dir, "%s,%s" % (dir_name, date_now.strftime("%H:%M:%S"))
        )
    ...
    def print_iters_summary(self, opt):
        ...
        strs += " Iter [{}/{}] Epoch [{}/{}]".format(
            iters, batch, epoch, self.args.epochs
        )

        if opt["split"] == "train":
            time_elapsed, time_total = self.get_time_info(
                epoch, iters, batch
            )  # Buggy for test
            strs += " Clock [{:.2f}h/{:.2f}h]".format(time_elapsed, time_total)
            strs += " LR [{}]".format(
                opt["recorder"].records[opt["split"]]["lr"][epoch][0]
            )
        self.print_write(strs)
    ...
    def print_epoch_summary(self, opt):
        ...
        epoch = opt["epoch"]
        self.print_write(
            "---------- {} Epoch {} Summary -----------".format(str.upper(split), epoch)
        )
    ...
    def save_img_results(self, results, split, epoch, iters, nrow, error=""):
        max_save_n = (
            self.args.test_save_n if split == "test" else self.args.train_save_n
        )
    ...
    def convert_to_same_size(self, t_list):
        ...
        for i, tensor in enumerate(t_list):
            ...
            if h == shape[2] and w == shape[3]:
                continue
            t_list[i] = torch.nn.functional.interpolate(
                tensor, [shape[2], shape[3]], mode="bilinear", align_corners=False
            )
        return t_list
    ...
    def save_normal_mat(self, normal_est, obj_name, save_dir):
        ...
        for i in range(batch):
            ...
            sio.savemat(
                os.path.join(save_dir, save_name + ".mat"),
                {"Normal_est": normal},
                do_compression=True,
            )
    ...
    def save_light(self, dirs, save_path):
        ...
        dirs = dirs.squeeze()
        np.savetxt(
            os.path.join(save_path[0], "GCNet_light_dirs_%s.txt" % self.date),
            dirs,
            fmt="%.6f",
        )
    ...
    def save_intens(self, ints, save_path):
        ...
        np.savetxt(
            os.path.join(save_path[0], "GCNet_light_ints_%s.txt" % self.date),
            ints,
            fmt="%.6f",
        )
        return ints
