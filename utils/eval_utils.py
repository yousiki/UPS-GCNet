...
def cal_ints_acc(gt_i, pred_i):
    ...
    scale = torch.linalg.lstsq(
        gt_i.view(-1, 1), pred_i.view(-1, 1)
    )  # NOTE: torch.gels is deprecated
...
def cal_normal_acc(gt_n, pred_n, mask=None):
    ...
    value = {
        "n_err_mean": n_err_mean.item(),  #'n_err_med': n_err_med.item(),
        "n_acc_11": n_acc_11.item(),
        "n_acc_30": n_acc_30.item(),
        "n_acc_45": n_acc_45.item(),
    }
