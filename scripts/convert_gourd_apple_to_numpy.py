...
)  # normalize images with light intensity
...
def exr_to_array(I, cname, mute=True):
    ...
    if not mute:
        print(
            "[Normalizing %s], max value: %f, min: %f, mean: %f"
...
def main(args):
    ...
    for d in dir_list:
        ...
        img_list = [
            os.path.join(args.input_dir, d, name_list[i]) for i in range(len(name_list))
        ]
        intens = None
        if args.normalize:
            intens = np.genfromtxt(
                os.path.join(args.input_dir, d, "light_intensities_refined.txt")
            )
