...
if __name__ == "__main__":
    ...
    name_list = utils.read_list(os.path.join(args.input_dir, "names.txt"))
    max_h, max_w = 0, 0
    crop_list = open(os.path.join(args.save_dir, "crop.txt"), "w")
    for d in dir_list:
        utils.make_file(os.path.join(args.save_dir, d))
        mask, normal, n_mat = load_mask_normal(d)
        l, r, t, b, h, w = get_bbox_compact(mask[:, :, 0] / 255)
        crop_list.write(
        ...
        print(
            "\t BBox L %d R %d T %d B %d, H:%d W:%d, Padded: %d %d"
            % (l, r, t, b, h, w, r - l, b - t)
        )
        ...
        sio.savemat(
            os.path.join(args.save_dir, d, "Normal_gt.mat"),
            {args.n_key: n_mat[t:b, l:r, :]},
            do_compression=True,
        )
        copy_txt(d)
