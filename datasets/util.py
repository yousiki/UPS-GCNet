...
def read_list(list_path, ignore_head=False, sort=True):
    lists = []
...
def light_source_directions():
    ...
    L = np.array(
        [
            [-0.06059872, -0.44839055, 0.8917812],
            ...
            [0.54551481, 0.36380988, 0.7550205],
        ],
        dtype=float,
    )
    return L
...
def exr_to_array(I, cname, mute=True):
    ...
    if cname == "normal":
        mask = np.expand_dims(img[:, :, 2] > 0, 2).repeat(3, 2)
        img[mask] = 0
        img[:, :, 2] = img[:, :, 2] * -1
        img[:, :, 0] = img[:, :, 0] * -1
    return img
