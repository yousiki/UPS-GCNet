...
class UpsSynthTestDataset(data.Dataset):
    def __init__(self, args, split="train"):
        ...
        self.split = split
        self.args = args
        self.shape_list = util.read_list(
            os.path.join(self.root, "%s_mtrl_%s.txt" % (split, self.args.syn_obj)),
            sort=False,
        )
        self.repeat = 1
    ...
    def _get_input_path(self, index):
        ...
        if hasattr(self.args, "light_index") and self.args.light_index != None:
            index_path = os.path.join(
                self.root, "Lighting_Index", self.args.light_index
            )
    ...
    def __getitem__(self, index):
        ...
        ratio = mask.sum().astype(np.float) / (
            mask.shape[0] * mask.shape[1]
        )  # ratio of object area in the whole image
        thres = 0.02
        if (
            img.mean() / ratio < thres
        ):  # if the mean value of the object region less than 0.02
        ...
        item["path"] = os.path.join(
            self.root, "Images", self.shape_list[index // self.repeat]
        )
